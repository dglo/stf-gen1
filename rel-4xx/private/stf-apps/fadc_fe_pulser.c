...
BOOLEAN fadc_fe_pulserEntry(STF_DESCRIPTOR *d,
  ...
  /*  5.Find if there is a reasonable value for the SPE discriminator level 
      such that will not trigger on noise.
      (Because part values vary from board to board and the noise band is 
      narrow
      the discriminator value must be scanned). The nominal value is obtained 
      from:
...
/*  7.Save the first SPE_DAC setting for which there is a Non-Zero value of the SPE rate meter.                        
  ...
  if (scanSPE(atwd_pedestal_dac, triggerable_spe_dac, 1)) {
    ...
    *fadc_fe_pulser_position = 0;
    memset(fadc_fe_pulser_waveform, 0, 512*sizeof(unsigned));
    free(waveform); 
    free(waveform_sum); 
    free(waveform_avg); 
    return FALSE;
